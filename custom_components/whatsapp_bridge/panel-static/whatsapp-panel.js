...
async function listContacts(hass) {
    const res = await hass.callWS({
        type: "whatsapp_bridge/contacts/list",
    });
    return res.contacts;
}
async function createContact(hass, data) {
    const res = await hass.callWS({
...
async function updateContact(hass, data) {
    const res = await hass.callWS({
...
let WhatsAppPanel = class WhatsAppPanel extends i {
    constructor() {
        super(...arguments);
        this.narrow = false;
        this._tab = "contacts";
    ...
    _renderPairing() {
        ...
        const qrSrc = state === "qr" && qrEntity?.attributes?.entity_picture
            ? qrEntity.attributes.entity_picture
            : null;
        const phone = status?.attributes?.phone ?? null;
        return b `
      <div class="pair">
        ...
        ${qrSrc
            ...
            `
            : state === "loading"
                ? b `<div class="pair-steps">WhatsApp Web is loading… this can take 30–60 seconds.</div>`
                : state === "disconnected"
                    ? b `
              <div class="pair-steps">
              ...
              <button class="btn" @click=${async () => this.hass && (await restartSession(this.hass))}>Restart client</button>
            `
                    : b `<div class="pair-steps">Waiting for the bridge to come online…</div>`}
        ${phone
            ? b `<div class="pair-steps">Linked: <code>${phone}</code></div>`
            : A}
      </div>
    `;
    }
    render() {
      ...
      <nav>
        ${["contacts", "conversations", "settings"].map((t) => b `
            <button
      ...
      <main>
        ...
        ${this._renderPairing()}
        ${this._tab === "contacts" ? this._renderContacts() : A}
        ${this._tab === "conversations" ? this._renderConversations() : A}
        ${this._tab === "settings" ? this._renderSettings() : A}
      </main>
      ${this._editing ? this._renderEditor() : A}
      ${this._sendTo ? this._renderSendDialog() : A}
    `;
    }
    _renderContacts() {
        return b `
      <div class="toolbar">
        ...
        <div class="actions">
          ...
          <button class="btn primary" @click=${() => (this._editing = { name: "", phone: "", notes: "" })}>Add contact</button>
        </div>
      </div>
      ${this._loading
            ? b `<p>Loading…</p>`
            ...
            <table>
              ...
              <tbody>
                ${this._contacts.map((c) => b `
                    <tr>
                      <td>${c.name}</td>
                      <td><code>${c.phone}</code></td>
                      <td>${c.notes || ""}</td>
                      <td class="actions">
                        <button class="btn" @click=${() => (this._sendTo = c)}>Send</button>
                        <button class="btn" @click=${() => (this._editing = {
                id: c.id,
                name: c.name,
                phone: c.phone,
                notes: c.notes,
    ...
    _renderSettings() {
      ...
      <div class="row" style="margin-top:16px">
        ...
        <button class="btn danger" @click=${async () => this.hass && confirm("Force re-pair with the phone?") && (await logoutSession(this.hass))}>
    ...
    _renderEditor() {
        ...
        return b `
      <div class="modal-backdrop" @click=${(e) => e.target === e.currentTarget && (this._editing = undefined)}>
        <div class="modal">
          <h3 style="margin:0">${draft.id ? "Edit contact" : "Add contact"}</h3>
          <label>Name
            <input .value=${draft.name}
              @input=${(e) => (this._editing = { ...draft, name: e.target.value })} />
          </label>
          <label>Phone
            <input .value=${draft.phone}
              placeholder="+972501234567"
              @input=${(e) => (this._editing = { ...draft, phone: e.target.value })} />
          </label>
          <label>Notes
            <textarea rows="3" .value=${draft.notes}
              @input=${(e) => (this._editing = { ...draft, notes: e.target.value })}></textarea>
          </label>
    ...
    _renderSendDialog() {
        ...
        return b `
      <div class="modal-backdrop" @click=${(e) => e.target === e.currentTarget && (this._sendTo = undefined)}>
        <div class="modal">
          <h3 style="margin:0">Send to ${c.name}</h3>
          <label>Message
            <textarea rows="4" .value=${this._sendBody}
              @input=${(e) => (this._sendBody = e.target.value)}></textarea>
          </label>
          <div class="actions" style="justify-content:flex-end">
    ...
    async _send() {
        if (!this.hass || !this._sendTo || !this._sendBody.trim())
            return;
        this._sending = true;
        try {
...
__decorate([
    n({ attribute: false })
], WhatsAppPanel.prototype, "hass", void 0);
__decorate([
    n({ attribute: false })
], WhatsAppPanel.prototype, "narrow", void 0);
__decorate([
    n({ attribute: false })
], WhatsAppPanel.prototype, "panel", void 0);
__decorate([
    r()
], WhatsAppPanel.prototype, "_tab", void 0);
__decorate([
    r()
], WhatsAppPanel.prototype, "_contacts", void 0);
__decorate([
    r()
], WhatsAppPanel.prototype, "_loading", void 0);
__decorate([
    r()
], WhatsAppPanel.prototype, "_error", void 0);
__decorate([
    r()
], WhatsAppPanel.prototype, "_editing", void 0);
__decorate([
    r()
], WhatsAppPanel.prototype, "_sendTo", void 0);
__decorate([
    r()
], WhatsAppPanel.prototype, "_sendBody", void 0);
__decorate([
    r()
], WhatsAppPanel.prototype, "_sending", void 0);
WhatsAppPanel = __decorate([
    t("whatsapp-panel")
], WhatsAppPanel);

export { WhatsAppPanel };
//# sourceMappingURL=whatsapp-panel.js.map
